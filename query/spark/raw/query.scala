...
//Levenshtein distance function for string similarity
  ...
  def main(args: Array[String]) {
    ...
    val input = sc.textFile("../../../cloud-data/reverbed.txt")
    val a = relation("-,born in,New York",input)
    
    //val data = input.filter(line => map2(line,"shahrukh khan","-","-","0","0"))
    //val dm = data.map(line => map4(line)).cache()
